...
function AutoSendCountdown({ minutesRemaining }: { minutesRemaining: number }) {
  ...
  return (
    ...
    >
      ...
      <span dir="ltr" style={{ fontSize: 12, fontWeight: 700, color: urgent ? "#dc2626" : "#92400e", fontFamily: "monospace" }}>
        {m}:{s.toString().padStart(2, "0")}
      </span>
      <span style={{ fontSize: 12, color: urgent ? "#dc2626" : "#92400e" }}>auto-send</span>
    </div>
  );
}
    ...
    >
      ...
      {isT4 && (
        ...
        >
          ...
          <span style={{ fontSize: 12, fontWeight: 800, color: "#fecaca", letterSpacing: "0.05em", textTransform: "uppercase" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
              <span style={{ ...ts, fontSize: 12, fontWeight: 700, padding: "2px 6px", borderRadius: 4, letterSpacing: "0.04em" }}>
                {conv.tier}
              </span>
              <span style={{ fontSize: 12, color: "#9ca3af" }}>{ts.label}</span>
              {!hideClientName && (
                <>
              ...
              <span style={{ fontSize: 12, color: "#d1d5db" }}>{conv.phone}</span>
              <span
                style={{
                  fontSize: 12, padding: "1px 5px", borderRadius: 99, fontWeight: 600,
            ...
            <div style={{ background: "#f9fafb", borderRadius: 7, padding: "8px 11px", marginBottom: 8 }}>
              <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 3 }}>Lead said:</div>
              <p dir="rtl" style={{ fontSize: 13, color: "#374151", margin: 0, textAlign: "right", lineHeight: 1.6 }}>
                {conv.lastMessage}
              </p>
            </div>
            ...
            {conv.draft && !editMode && (
              <div style={{ background: "#eef2ff", border: "1px solid #c7d2fe", borderRadius: 7, padding: "8px 11px", marginBottom: 8, marginTop: (!isAutoQueued && schedulingCtx) ? 6 : 0 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: "#4F46E5" }}>AI Draft</span>
                  {/* Confidence bar */}
                  {conv.confidence > 0 && (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2 }}>
                      <span style={{ fontSize: 12, color: confColor, fontWeight: 700 }}>{confPct}%</span>
                ...
                <p dir="rtl" style={{ fontSize: 13, color: "#312e81", margin: 0, textAlign: "right", lineHeight: 1.6 }}>
                  {conv.draft}
                </p>
            ...
            {conv.draft && editMode && (
              <div style={{ marginBottom: 8 }}>
                <textarea
                  ...
                  rows={3}
                  style={{
                    width: "100%", fontSize: 13, padding: "8px 11px", borderRadius: 7,
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 5 }}>
                  <button onClick={() => { onApprove(); setEditMode(false); }} style={{ fontSize: 12, padding: "4px 10px", background: "#4F46E5", color: "#fff", border: "none", borderRadius: 5, cursor: "pointer", fontWeight: 600 }}>
                    Send Edited
                  </button>
                  <button onClick={() => setEditMode(false)} style={{ fontSize: 12, padding: "4px 10px", background: "#f3f4f6", color: "#6b7280", border: "none", borderRadius: 5, cursor: "pointer" }}>
                    Cancel
                  </button>
                </div>
              </div>
            ...
            {isHuman && !allCleared && (
              <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 7, padding: "8px 11px", marginBottom: 8 }}>
                ...
                {conv.takeoverAt && (
                  <p style={{ fontSize: 12, color: "#f87171", margin: "2px 0 0" }}>Since {conv.takeoverAt}</p>
                )}
            ...
            {isHuman && allCleared && (
              <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 7, padding: "8px 11px", marginBottom: 8 }}>
                <p style={{ fontSize: 12, color: "#16a34a", fontWeight: 600, margin: 0 }}>✓ All clear — AI resuming with:</p>
                <p dir="rtl" style={{ fontSize: 12, color: "#166534", margin: "4px 0 0", textAlign: "right", background: "#dcfce7", borderRadius: 5, padding: "4px 7px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0, minWidth: 90 }}>
            <span style={{ fontSize: 12, color: "#9ca3af" }}>{conv.age}</span>
            <span style={{ fontSize: 12, padding: "1px 6px", borderRadius: 99, fontWeight: 600, background: trustStyle.bg, color: trustStyle.color }}>
              {conv.trustLevel}
            </span>
            ...
            {conv.draft && !editMode && (
              <div style={{ display: "flex", flexDirection: "column", gap: 4, width: "100%" }}>
                <button
                  onClick={onApprove}
                  style={{ fontSize: 12, padding: "5px 0", background: "#22c55e", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600 }}
                >
                  ✓ Approve
                </button>
                <button
                  onClick={() => setEditMode(true)}
                  style={{ fontSize: 12, padding: "5px 0", background: "#fff", border: "1px solid #e5e7eb", color: "#374151", borderRadius: 6, cursor: "pointer" }}
                >
                  ✏ Edit
                </button>
                <button
                  onClick={onReject}
                  style={{ fontSize: 12, padding: "5px 0", background: "#fff", border: "1px solid #fca5a5", color: "#dc2626", borderRadius: 6, cursor: "pointer" }}
                >
                  ✗ Reject
                </button>
              </div>
            ...
            {isHuman && !allCleared && (
              <button
                ...
                style={{ fontSize: 12, padding: "5px 10px", background: "#16a34a", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600, whiteSpace: "nowrap" }}
              >
                ✓ All Clear
              </button>
            )}
            ...
            {isAutoQueued && !editMode && (
              <button
                onClick={onReject}
                style={{ fontSize: 12, padding: "4px 8px", background: "#fff7ed", color: "#c2410c", border: "1px solid #fed7aa", borderRadius: 5, cursor: "pointer" }}
              >
                Cancel Auto
              </button>
            )}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <select
              ...
              style={{ fontSize: 12, border: "1px solid #e5e7eb", borderRadius: 6, padding: "5px 9px", background: "#fff", color: "#374151" }}
            >
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          {(["T4", "T3", "T2", "T1"] as const).map((tier) => {
            ...
            return (
              ...
              >
                <span style={{ ...s, fontSize: 12, fontWeight: 700, padding: "1px 5px", borderRadius: 4 }}>{tier}</span>
                <span style={{ fontSize: 12, color: "#6b7280" }}>{s.label}</span>
                <span className="num-display" style={{ fontSize: 14, fontWeight: 700, color: count > 0 ? s.color : "#d1d5db" }}>{count}</span>
                {tier !== "T1" && count === 0 && <span style={{ fontSize: 12, color: "#d1d5db" }}>clear</span>}
              </div>
            );
          })}
          {tierFilter !== "All Tiers" && (
            <button
              ...
              style={{ fontSize: 12, padding: "5px 10px", background: "none", border: "1px solid #e5e7eb", borderRadius: 7, color: "#6b7280", cursor: "pointer" }}
            >
              × Clear filter
            </button>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {hasAttention && (
              <div
                style={{
                  ...
                  padding: "6px 14px", fontSize: 12, fontWeight: 700, color: "#dc2626",
                  borderRadius: 4,
                }}
              >
            ...
            {groups.length === 0 && (
              <div style={{ textAlign: 'center', padding: '48px 16px', color: 'var(--text-muted)' }}>
                <svg width="40" height="40" style={{ margin: '0 auto 12px', display: 'block', opacity: 0.4 }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>
                <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 4 }}>תור ריק</p>
                <p style={{ fontSize: 12 }}>כל ההודעות אושרו — כל הכבוד!</p>
              </div>
            )}
                ...
                >
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ fontSize: 12, background: "#eef2ff", color: "#4F46E5", fontWeight: 700, padding: "1px 6px", borderRadius: 99 }}>
                        {convs.length}
                      </span>
                      {hasT4 && (
                        <span style={{ fontSize: 12, background: "#fef2f2", color: "#dc2626", fontWeight: 700, padding: "1px 6px", borderRadius: 99 }} className="pill-pulse">
                          T4 CRISIS
                        </span>
                      )}
                    </div>
                    <div style={{ display: "flex", gap: 6 }}>
                      {t1Count > 0 && (
                        <button
                          ...
                          style={{ fontSize: 12, padding: "3px 9px", background: "#4F46E5", color: "#fff", border: "none", borderRadius: 5, cursor: "pointer", fontWeight: 600 }}
                        >
                          Approve T1 ({t1Count})
                        </button>
                      )}
              ...
              <>
                <div
                  style={{
                    ...
                    padding: "6px 14px", fontSize: 12, fontWeight: 700, color: "#6b7280",
                    borderRadius: 4,
                  }}
                >
