      ...
      {DIM_CONFIG.map((d) => {
        ...
        return (
          <div key={d.key} style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontSize: compact ? 9 : 12, fontWeight: 700, color, width: 24, textAlign: "right" }}>{val}</span>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: 14, fontWeight: 700, color: cfg.dot, lineHeight: 1 }}>{score}</span>
      </div>
    </div>
  );
}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
            <Link href="/clients" style={{ fontSize: 12, color: "#9ca3af", textDecoration: "none" }}>Clients</Link>
            <span style={{ fontSize: 12, color: "#d1d5db" }}>›</span>
            <span style={{ fontSize: 12, color: "#374151" }}>Health Dashboard</span>
          </div>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 12, color: "#9ca3af" }}>Agency Health</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: agencyHealthScore >= 75 ? "#16a34a" : agencyHealthScore >= 50 ? "#d97706" : "#dc2626" }}>
            {agencyHealthScore}
          </div>
        </div>
      </div>
        ...
        {(["green", "yellow", "orange", "red"] as ChurnTier[]).map((tier) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: cfg.dot, lineHeight: 1.1 }}>
                {tierCounts[tier]}
              </div>
        ...
        return (
          <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 10, padding: "12px 16px", marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 12, color: "#3b82f6" }}>— {quietHealthy.length} quiet, healthy clients who haven&apos;t heard from you lately</span>
            </div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {quietHealthy.map((c) => (
                <div key={c.id} style={{ background: "#fff", border: "1px solid #bfdbfe", borderRadius: 7, padding: "7px 12px", display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: "#6b7280" }}>{c.lastInteractionDays}d no touch</span>
                  <button style={{ fontSize: 12, padding: "2px 7px", borderRadius: 4, border: "none", background: "#1d4ed8", color: "#fff", cursor: "pointer", fontWeight: 600 }}>
                    Send Update
                  </button>
                </div>
              ))}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 0, background: "#fff", borderRadius: 10, border: "1px solid #e5e7eb", overflow: "hidden" }}>
        <div style={{ padding: "10px 16px", borderBottom: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h2 style={{ fontSize: 13, fontWeight: 600, color: "#111827", margin: 0 }}>
            ...
            {selectedTier !== "all" && (
              <span style={{ marginLeft: 8, fontSize: 12, color: TIER_CONFIG[selectedTier].dot }}>
                — {TIER_CONFIG[selectedTier].label} only
              </span>
            )}
          </h2>
          {selectedTier !== "all" && (
            <button onClick={() => setSelectedTier("all")} style={{ fontSize: 12, color: "#9ca3af", background: "none", border: "none", cursor: "pointer" }}>
              Show all
            </button>
          )}
        </div>
        {filtered.map((client, i) => {
          ...
          return (
            <div key={client.id} style={{ borderBottom: i < filtered.length - 1 ? "1px solid #f3f4f6" : "none" }}>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, padding: "1px 5px", borderRadius: 3, background: tierCfg.bg, color: tierCfg.color }}>{tierCfg.label}</span>
                    <span style={{ fontSize: 12, color: "#9ca3af" }}>{client.domain}</span>
                  </div>
                  {warnings.length > 0 && (
                    <div style={{ display: "flex", gap: 5, marginBottom: 4 }}>
                      {warnings.map((w) => (
                        <span key={w} style={{ fontSize: 12, padding: "1px 6px", borderRadius: 3, background: "#fef2f2", color: "#dc2626", fontWeight: 600 }}>⚠ {w}</span>
                      ))}
                    </div>
                  )}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4, flexShrink: 0, textAlign: "right" }}>
                  <div>
                    <div style={{ fontSize: 12, color: "#9ca3af" }}>Monthly Budget</div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: "#111827" }}>₪{client.monthlyBudget.toLocaleString()}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 12, color: "#9ca3af" }}>Renewal</div>
              ...
              {expanded && (
                <div style={{ padding: "0 16px 16px 16px", borderTop: "1px solid #f9fafb" }}>
                  <div className="responsive-grid-4" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div>
                      <div style={{ fontSize: 12, color: "#9ca3af", fontWeight: 600, marginBottom: 8 }}>HEALTH DIMENSIONS — FULL VIEW</div>
                      <RadarBars dims={client.healthDimensions} />
                    </div>
                    <div>
                      <div style={{ fontSize: 12, color: "#9ca3af", fontWeight: 600, marginBottom: 8 }}>KEY METRICS</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {[
                          ...
                          <div key={m.label} style={{ display: "flex", justifyContent: "space-between" }}>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 600, color: "#111827" }}>{m.value}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {client.churnTier === "red" || client.churnTier === "orange" ? (
                    <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
                      <button style={{ fontSize: 12, padding: "5px 12px", borderRadius: 5, border: "none", background: "#dc2626", color: "#fff", cursor: "pointer", fontWeight: 600 }}>
